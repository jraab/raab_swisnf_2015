...
annotatePeaks <- function(peaks, annotation, window, peak_point = 'center')  { 
  ...
  if (peak_point == 'center') { 
    peaks <- with(peaks, GRanges(seqnames=seqnames,
                                  ...
                                  strand = strand, elementMetadata(peaks)) )
  }
...
# -----------------------------------------------------------------------------
annotateChromState <- function(peaks, states) { 
  # annotate a peaks or some bed grange object with the chromatin states
  # states should be grange object converted from chromHMM output 
  # expectation is that all peaks will overlap 1 state
  # Return the name of the overlapping state (if no state overlaps for some reason)
  #  The none category is due to a few telomeric or centromeric peaks that have no 
  #  associated state for some reason ( I think because this was lifted over from Hg18)
  # Return 'NONE'
  
  #shrink peak to midpoint
  peaks2 <- with(peaks, GRanges(seqnames = seqnames, 
                        IRanges(start = floor(start+end)/2, width =2), 
                        strand = strand, elementMetadata(peaks) ) )
  
  #ugly - but I couldnt think of a clearer way to keep things in order
  states2 <- ifelse(overlapsAny(peaks2, states), states[subjectHits(findOverlaps(peaks2, states)),]$name, 'NONE')  
  peaks$state <- states2
  return(peaks)
}
